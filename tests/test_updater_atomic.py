...
def test_atomic_helper_uses_neutral_lock_and_environment_names():
    helper = Path("scripts/system-update-updater-atomic.py").read_text(encoding="utf-8")

    assert "/run/finance-reconciliation-system-update-updater.lock" in helper
    assert "FINREC_UPDATER_IMAGE_TAG" in helper


def test_atomic_helper_uses_neutral_same_directory_temporary_prefix():
    helper = Path("scripts/system-update-updater-atomic.py").read_text(encoding="utf-8")

    assert 'prefix=".finance-reconciliation-system-update-updater."' in helper
