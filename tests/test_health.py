...
def test_wait_for_database_returns_after_successful_connection(monkeypatch):
    connect = MagicMock()
    database_url = (
        "postgresql://finance:finance_dev@db:5432/finance_reconciliation"
    )
...
@pytest.mark.parametrize(
    ("name", "value"),
    [
        ("FINREC_UPDATER_URL", None),
        ("FINREC_UPDATER_URL", "http://updater:8090/path"),
        ("FINREC_UPDATER_URL", "http://token@updater:8090"),
        ("FINREC_UPDATER_URL", "http://updater:8090?next=unsafe"),
        ("FINREC_UPDATER_TOKEN", None),
        ("FINREC_UPDATER_TOKEN", " " * 32),
        ("FINREC_UPDATER_TOKEN", "short-token"),
    ],
)
def test_production_settings_require_strict_updater_configuration(monkeypatch, name, value):
    ...
    with pytest.raises(ImproperlyConfigured, match="FINREC_UPDATER"):
        importlib.import_module("config.settings.prod")
...
def test_production_settings_expose_only_fixed_updater_endpoint(monkeypatch):
    ...
    assert production_settings.FINREC_UPDATER_URL == "http://updater:8090"
    assert production_settings.FINREC_UPDATER_TOKEN == "u" * 32
...
def test_production_settings_reject_noncanonical_release_versions(
    ...
    _configure_production_environment(monkeypatch)
    monkeypatch.setenv("FINREC_RELEASE_VERSION", release_version)
    sys.modules.pop("config.settings.prod", None)
...
def test_production_release_version_uses_finrec_prefix(monkeypatch):
    ...
    assert production_settings.FINREC_RELEASE_VERSION == "v12.34.56"
