...
def _write_xlsx(path: Path, rows: list[list[object]]) -> None:
    workbook = Workbook()
    workbook.properties.creator = "Finance Reconciliation synthetic fixture generator"
...
def _normalize_xlsx_archive(path: Path) -> None:
    ...
    with ZipFile(path, "w", compression=ZIP_DEFLATED) as destination:
        for filename, compress_type, content in entries:
            info = ZipInfo(filename, date_time=FIXTURE_ZIP_TIMESTAMP)
...
def _write_general_bank_fixtures(fixtures: Path) -> None:
...
def _write_invoice_fixtures(fixtures: Path, railway_fixtures: Path) -> None:
    input_row = [
        "INPUT-001",
        "913200000000000001",
        "测试供应商",
...
def _write_railway_bank_fixture(railway_fixtures: Path) -> None:
...
def _write_invoice_pdf(fixtures: Path) -> None:
    writer = PdfWriter()
...
def generate_synthetic_fixtures(output_dir: Path) -> None:
    railway_fixtures = output_dir / "synthetic_railway"
    railway_fixtures.mkdir(parents=True, exist_ok=True)
    _write_general_bank_fixtures(output_dir)
    _write_invoice_fixtures(output_dir, railway_fixtures)
    _write_railway_bank_fixture(railway_fixtures)
    _write_invoice_pdf(output_dir)


if __name__ == "__main__":
    generate_synthetic_fixtures(FIXTURES)
