...
def test_from_env_rejects_missing_blank_or_short_private_token(fixed_paths, token):
    values = environment(FINREC_UPDATER_TOKEN=token)
...
def test_config_uses_fixed_neutral_platform(fixed_paths):
    with pytest.raises(config.ConfigError):
        config.UpdaterConfig.from_env(environment(FINREC_COMPOSE_PROJECT="other"))
    ...
    assert result.platform.web_repository == "ghcr.io/s450586793/finance-reconciliation-web"
...
@pytest.mark.parametrize(
    ...
    [
        "FINREC_COMPOSE_FILE",
        "FINREC_ENV_FILE",
        "FINREC_UPDATE_STATE_FILE",
        "FINREC_WEB_REPOSITORY",
        "FINREC_UPDATER_LISTEN",
    ],
)
