...
def _target_env(tmp_path: Path) -> Path:
    path = tmp_path / ".env"
    path.write_text("FINREC_WEB_IMAGE_TAG=v1.3.0\n")
    path.chmod(0o600)
    return path
...
@pytest.mark.parametrize(
    ...
    [
        b"OTHER=value\n",
        b"FINREC_WEB_IMAGE_TAG=v1.2.3\nFINREC_WEB_IMAGE_TAG=v1.2.3\n",
        b"DUP=value\nDUP=other\nFINREC_WEB_IMAGE_TAG=v1.2.3\n",
        b"FINREC_WEB_IMAGE_TAG=latest\n",
    ],
)
...
def test_persist_version_rejects_non_private_or_symlink_environment_file(
    ...
    real.write_text("FINREC_WEB_IMAGE_TAG=v1.2.3\n")
    real.chmod(0o644)
