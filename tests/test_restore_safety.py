...
PRODUCTION_URL = "postgresql://finance:secret@production-db:5432/finance_reconciliation"
...
@pytest.mark.parametrize(
    "restore_url",
    [
        "postgresql://restore:secret@restore-db:5432/safe_restore?dbname=finance_reconciliation",
        "postgres://restore:secret@production-alias:5432/finance_reconciliation?application_name=drill",
        "postgresql://restore:secret@production-db.:5432/finance_reconciliation?connect_timeout=2",
        "service=finance_restore",
...
def test_restore_target_accepts_distinct_named_restore_database():
    validate_restore_target(
        PRODUCTION_URL,
        "postgresql://restore:secret@restore-db:5432/finance_restore?application_name=drill",
        environment={},
    )
    ...
    environment.update(
        {
            ...
            "RESTORE_DATABASE_URL": "postgresql://restore:secret@restore-db:5432/finance_restore",
            name: secret,
        }
    )
...
def test_restore_target_accepts_explicit_empty_environment(monkeypatch):
    ...
    validate_restore_target(
        PRODUCTION_URL,
        "postgresql://restore:secret@restore-db:5432/finance_restore",
        environment={},
    )
...
def test_pg_restore_helper_starts_with_allowlisted_environment(tmp_path):
    ...
    assert "--dbname=postgresql://restore:secret@restore-db:5432/finance_restore" in (
        captured_arguments
    )
