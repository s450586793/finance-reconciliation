...
services:
  db:
    ...
    environment:
      POSTGRES_DB: ${POSTGRES_DB:-finance_reconciliation}
      POSTGRES_USER: ${POSTGRES_USER:-finance}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-finance_dev}
    volumes:
...
services:
  web:
    ...
    volumes:
      - /volume4/docker/docker/finance-reconciliation/uploads:/data/uploads
      - /volume4/docker/docker/finance-reconciliation/exports:/data/exports
      - /volume4/docker/docker/finance-reconciliation/backups:/data/backups
  db:
    restart: unless-stopped
    volumes:
      - /volume4/docker/docker/finance-reconciliation/postgres:/var/lib/postgresql/data
```
...
test('finance imports and partially reconciles while owner stays read-only', async ({ page }) => {
  ...
  await expect(page.getByText('目标企业财务驾驶舱')).toBeVisible();
