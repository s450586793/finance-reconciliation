...
**Goal:** 为财务对账系统交付仅老板可操作的 Web 手动升级能力，通过 GitHub/GHCR 发布不可变镜像，在 DSM 上先备份、失败自动回滚，并在成功后精准删除本次旧 Web 镜像。
...
- 源码仓库为公开 GitHub `s450586793/finance-reconciliation`，公开仓库和镜像层不得包含 `.env`、生产数据、附件、备份、Token、Cookie、私钥或账号口令。
- release 只接受规范 `vX.Y.Z`；Web/updater 都发布不可变版本标签，两个镜像全部成功后才能移动 Web `stable`。
- updater 不使用 `stable`，DSM 必须通过 `FINREC_UPDATER_IMAGE_TAG=vX.Y.Z` 固定版本，并且只能人工升级 updater。
- 只有 updater 挂载 `/var/run/docker.sock`；Web 不得挂载 Docker socket，updater 不得发布宿主机端口。
- Web 页面只升级 Compose project `finance-reconciliation` 的 `web` 服务，不得替换 `db` 或 updater。
...
- Produces: `settings.FINREC_RELEASE_VERSION`, validated as canonical `vX.Y.Z` in production。
...
def test_production_release_version_is_canonical(monkeypatch):
    ...
    monkeypatch.setenv("FINREC_RELEASE_VERSION", "latest")
    with pytest.raises(ImproperlyConfigured, match="FINREC_RELEASE_VERSION"):
...
- `docker compose --project-name finance-reconciliation --env-file /config/.env -f /config/compose.yml ps -q web`;
- container and image inspect with strict JSON parsing;
- `docker pull ghcr.io/s450586793/finance-reconciliation-web:stable` followed by exact digest/OCI label inspection;
...
- version `.env` atomic replacement changing exactly one `FINREC_WEB_IMAGE_TAG` line and preserving mode `0600`;
...
class PlatformConfig:
    project_name: str = "finance-reconciliation"
    compose_file: Path = Path("/config/compose.yml")
    env_file: Path = Path("/config/.env")
    web_repository: str = "ghcr.io/s450586793/finance-reconciliation-web"
...
The only valid rollback alias is `finance-reconciliation-rollback-web:<canonical-task-uuid>`. Cleanup removes only the exact original version tags recorded before the switch, then that task alias, then the verified image ID; a missing tag may be ignored only when image inspect proves it no longer points at the original ID.
...
Require non-whitespace Token of at least 32 UTF-8 bytes, exact `finance-reconciliation` project, `/config/compose.yml`, `/config/.env`, `/state/update-state.json`, fixed GHCR repository and loopback-safe listen `:8090`. Reject path traversal, symlinks escaping `/config` or `/state`, alternate repositories and arbitrary listen hosts.
...
class UpdaterConfig:
    ...
    def from_env(cls, environ: Mapping[str, str]) -> "UpdaterConfig":
        token = require_private_token(environ, "FINREC_UPDATER_TOKEN", minimum_bytes=32)
        require_exact(environ, "FINREC_COMPOSE_PROJECT", "finance-reconciliation")
        return cls(
            token=token,
            listen=("0.0.0.0", 8090),
...
Require `FINREC_UPDATER_URL` and Token together in production, URL exactly `http://updater:8090`, no query/userinfo/fragments, and Token non-whitespace >= 32 bytes. Mock HTTP for timeout, refusal, 401/409/503, oversized response, unknown JSON field, wrong enum and raw-secret body.
...
- Web image default is `ghcr.io/s450586793/finance-reconciliation-web:${FINREC_WEB_IMAGE_TAG}` and has no build section in production Compose;
- updater image uses required immutable `FINREC_UPDATER_IMAGE_TAG`;
...
- updater mounts `${FINREC_APP_DIR}` at `/config` and `${FINREC_DATA_DIR}/updater-state` at `/state`;
- db/web/updater share one internal network, Web depends on healthy db, updater health is internal;
- only Web carries `FINREC_RELEASE_VERSION`, only Django/updater carry the internal Token;
...
ARG FINREC_RELEASE_VERSION
ARG FINREC_RELEASE_REVISION
ARG FINREC_RELEASE_CREATED
LABEL org.opencontainers.image.version=$FINREC_RELEASE_VERSION \
      org.opencontainers.image.revision=$FINREC_RELEASE_REVISION \
      org.opencontainers.image.created=$FINREC_RELEASE_CREATED
ENV FINREC_RELEASE_VERSION=$FINREC_RELEASE_VERSION
...
`.env.example` must require `FINREC_APP_DIR`, `FINREC_DATA_DIR`, `FINREC_WEB_IMAGE_TAG`, `FINREC_UPDATER_IMAGE_TAG`, `FINREC_UPDATER_TOKEN`, and existing production values. `deploy-dsm.sh` validates exact versions and >=32-byte Token, renders Compose, pulls Web/updater, starts the three services, runs migrations for first deployment, waits for database-aware Web and updater health, and never prints secrets.
...
services:
  ...
  updater:
    ...
    volumes:
      ...
      - ${FINREC_APP_DIR:?required}:/config
      - ${FINREC_DATA_DIR:?required}/updater-state:/state
```
...
Run: `docker build --target web --build-arg FINREC_RELEASE_VERSION=v0.2.0 --build-arg FINREC_RELEASE_REVISION=$(git rev-parse HEAD) --build-arg FINREC_RELEASE_CREATED=2026-08-06T00:00:00Z .`
...
**Interfaces:**
- Produces public release workflow for `ghcr.io/s450586793/finance-reconciliation-web` and `ghcr.io/s450586793/finance-reconciliation-updater`。
...
**Interfaces:**
- Produces guarded DSM acceptance commands requiring `FINREC_CONFIRM_SYSTEM_UPDATE=yes` and separate `FINREC_CONFIRM_ROLLBACK_SMOKE=yes`。
...
[[ "${FINREC_EXPECTED_TARGET:-}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]] || exit 2
cookie_jar="$(mktemp)"
...
fetch_public_status() {
  curl --fail --silent --show-error --max-time 5 \
    --cookie "$cookie_jar" \
    "${FINREC_BASE_URL}/system/update/status/"
}
...
9. preserving external `http://sd.ace-station.top:1111` routing.
...
- [ ] Compare every requirement in `docs/superpowers/specs/2026-08-06-web-managed-upgrades-design.md` with Tasks 1-12 and identify its implementation/test owner.
