...
class DockerPlatform:
    ...
    def persist_version(self, version: str) -> None:
        try:
            ...
            if _WEB_TAG_KEY not in values:
                raise ValueError("missing_web_version")
            validate_version(values[_WEB_TAG_KEY])
            replacement_count = 0
            ...
            for line in content.splitlines(keepends=True):
                ...
                if match is not None and match.group(1) == _WEB_TAG_KEY:
                    updated_lines.append(f"{_WEB_TAG_KEY}={validated_version}{ending}")
                    replacement_count += 1
                else:
    ...
    def cleanup_original_step(
        ...
        try:
            ...
            if environment.get(_WEB_TAG_KEY) != task.target.version:
    ...
    def _is_original(self, original: ImageIdentity, *, alias_optional: bool = False) -> bool:
        ...
        expected_alias = re.fullmatch(
            r"finance-reconciliation-rollback-web:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[89ab][0-9a-f]{3}-[0-9a-f]{12}",
