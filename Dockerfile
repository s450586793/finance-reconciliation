FROM python:3.12.11-alpine3.21 AS web

ARG FINREC_RELEASE_VERSION
ARG FINREC_RELEASE_REVISION
ARG FINREC_RELEASE_CREATED

LABEL org.opencontainers.image.version=$FINREC_RELEASE_VERSION \
      org.opencontainers.image.revision=$FINREC_RELEASE_REVISION \
      org.opencontainers.image.created=$FINREC_RELEASE_CREATED

ENV PYTHONDONTWRITEBYTECODE=1 \
    PYTHONUNBUFFERED=1 \
    FINREC_RELEASE_VERSION=$FINREC_RELEASE_VERSION

WORKDIR /app

COPY pyproject.toml manage.py /app/
COPY config /app/config
COPY apps /app/apps
COPY scripts /app/scripts
COPY templates /app/templates
COPY static /app/static

RUN pip install --no-cache-dir --upgrade pip \
    && pip install --no-cache-dir "." \
    && apk add --no-cache ca-certificates curl postgresql16-client \
    && addgroup -S -g 10001 app \
    && adduser -S -D -H -u 10001 -G app app \
    && chmod +x /app/scripts/*.sh \
    && chown -R app:app /app

USER app

RUN build_secret="$(python -c 'from django.core.management.utils import get_random_secret_key; print(get_random_secret_key())')" \
    && build_password="$(python -c 'import secrets; print(secrets.token_hex(32))')" \
    && DJANGO_SETTINGS_MODULE=config.settings.prod \
       DJANGO_SECRET_KEY="$build_secret" \
       DJANGO_ALLOWED_HOSTS=build.invalid \
       CSRF_TRUSTED_ORIGINS=https://build.invalid \
       COMPANY_TAX_ID=91320281MA00000001 \
       DATABASE_URL="postgresql://build:$build_password@db:5432/build" \
       FINREC_RELEASE_VERSION="${FINREC_RELEASE_VERSION:-v0.0.0}" \
       FINREC_UPDATER_URL=http://updater:8090 \
       FINREC_UPDATER_TOKEN="$(python -c 'print("u" * 32)')" \
       python manage.py collectstatic --noinput

CMD ["gunicorn", "config.wsgi:application", "--bind", "0.0.0.0:8000", "--workers", "3", "--timeout", "120"]

FROM docker:27.5.1-cli AS updater

RUN apk add --no-cache ca-certificates curl docker-cli-compose python3

COPY updater /app/updater

WORKDIR /app

ENTRYPOINT ["python3", "-m", "updater.main"]
