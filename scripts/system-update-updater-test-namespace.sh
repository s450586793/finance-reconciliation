...
if [ "${1:-}" = "--inside" ]; then
  ...
  /usr/bin/install -d -o root -g root -m 0711 "$rootfs/volume4/.finance-reconciliation-test-bin"
  /usr/bin/install -o root -g root -m 0755 "$real_python" "$rootfs/volume4/.finance-reconciliation-test-bin/python3"
  /usr/bin/install -o root -g root -m 0755 /usr/bin/mktemp "$rootfs/volume4/.finance-reconciliation-test-bin/mktemp"
  /usr/bin/install -o root -g root -m 0755 /usr/bin/rm "$rootfs/volume4/.finance-reconciliation-test-bin/rm"
  /usr/bin/rm -f \
  ...
  /usr/bin/install -d -o root -g root -m 0700 "$rootfs/volume4/.finance-reconciliation-updater-fake"
  ...
  read -r \
    ...
    log_identity \
    state_identity <<<"$(
    /usr/sbin/chroot "$rootfs" /volume4/.finance-reconciliation-test-bin/python3 -I -S - \
      /volume4/.finance-reconciliation-updater-fake \
      /var/run/docker.sock \
...
PY
  ...
  for docker_candidate in "${docker_candidates[@]}"; do
    /usr/sbin/chroot "$rootfs" /usr/bin/env -i \
      ...
      FINREC_FAKE_DOCKER_PREFLIGHT=1 \
      "$docker_candidate" \
      --host unix:///var/run/docker.sock \
      pull ghcr.io/s450586793/finance-reconciliation-updater:v1.2.4 \
      >/dev/null 2>&1
  done
...
try:
    ...
    os.close(directory_descriptor)
PY
  /usr/bin/chown "$target_uid:$target_gid" "$rootfs/volume4/.finance-reconciliation-updater-fake"
  /usr/bin/chmod 0700 "$rootfs/volume4/.finance-reconciliation-updater-fake"
  ...
  (
    ...
    /usr/sbin/chroot "$rootfs" \
      ...
      /usr/bin/env -i \
        HOME=/tmp \
        PATH=/usr/bin:/usr/local/bin:/bin \
        FINREC_TEST_NAMESPACE=1 \
        FINREC_TEST_PRIVATE_ROOTFS=1 \
        FINREC_TEST_BATCH="$test_batch" \
...
lock_path = Path("/run/finance-reconciliation-system-update-updater-test.lock")
marker_name = ".finance-reconciliation-updater-rootfs-owner"
...
try:
    ...
    while rootfs_path is None:
        candidate = Path("/run") / f"finance-reconciliation-system-update-updater-rootfs.{secrets.token_hex(16)}"
        try:
