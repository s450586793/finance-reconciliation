...
LOG_PATH = Path(os.environ["FAKE_LOG"])
COUNTERS_PATH = Path(os.environ["FAKE_COUNTERS"])
WEB_REPOSITORY = "ghcr.io/s450586793/finance-reconciliation-web"
...
def image_inventory(index: int) -> list[dict[str, str]]:
    ...
    alias_row = image_row("finance-reconciliation-rollback-web", task_id, OLD_IMAGE_ID)
    ...
    if SCENARIO.startswith("cleanup_pending_") or SCENARIO == "success_pending":
        ...
        if SCENARIO == "cleanup_pending_missing_tag":
            ...
            rows[1] = image_row("finance-reconciliation-rollback-web", task_id, TARGET_IMAGE_ID)
...
def container_payload(container_id: str) -> dict[str, object]:
    ...
    if container_id == "updater-keep":
        ...
        return {
            ...
            "State": {"StartedAt": started_at},
            "Config": {
                "Image": "ghcr.io/s450586793/finance-reconciliation-updater:v0.2.0",
                "Labels": {
                    "com.docker.compose.project": "finance-reconciliation",
    ...
    oci_version = "v0.2.1"
    compose_project = "finance-reconciliation"
    compose_service = "web"
...
def state_payload() -> dict[str, object]:
    ...
    return {
        ...
        "task": {
            "id": task_id,
            "original": {
                "repository": "ghcr.io/s450586793/finance-reconciliation-web",
                "version": "v0.2.0",
                "digest": OLD_DIGEST,
                "image_id": OLD_IMAGE_ID,
                "tags": original_tags,
                "rollback_alias": f"finance-reconciliation-rollback-web:{task_id}",
...
def main() -> int:
    ...
    if args[:2] == ["image", "inspect"] and len(args) == 3:
        if args[2] == "ghcr.io/s450586793/finance-reconciliation-web:v0.2.0":
            sys.stdout.write(json.dumps([{"Id": OLD_IMAGE_ID}]))
            return 0
        if args[2] == "ghcr.io/s450586793/finance-reconciliation-web:v0.2.1":
...
PY
  ...
  set +e
  env \
    FINREC_BASE_URL="${FINREC_BASE_URL-http://example.test:1111}" \
    FINREC_APP_DIR="${FINREC_APP_DIR-/safe/app}" \
    FINREC_OWNER_USERNAME="${FINREC_OWNER_USERNAME-owner-safe}" \
    FINREC_OWNER_PASSWORD="${FINREC_OWNER_PASSWORD-owner-password-safe-2026}" \
    FINREC_EXPECTED_TARGET="${FINREC_EXPECTED_TARGET-v0.2.1}" \
    FINREC_CONFIRM_SYSTEM_UPDATE="${FINREC_CONFIRM_SYSTEM_UPDATE-yes}" \
    FINREC_CONFIRM_ROLLBACK_SMOKE="${FINREC_CONFIRM_ROLLBACK_SMOKE-}" \
    FINREC_SMOKE_MODE="${FINREC_SMOKE_MODE-success}" \
...
forbidden = (
    ...
    "Traceback",
    "JSONDecodeError",
    "finance-reconciliation-rollback-web:",
    "/safe/app",
    "db-keep",
    "updater-keep",
    "web-old",
...
for forbidden in (
    ...
    "web-old",
    "web-target",
    f"finance-reconciliation-rollback-web:{task_id}" if task_id else "never-present-private-alias",
...
unset FINREC_CONFIRM_SYSTEM_UPDATE
FINREC_BASE_URL=""
run_case "no_actions" 2
export LAST_LOG_FILE LAST_STDOUT_FILE LAST_STDERR_FILE
assert_contains "$LAST_STDERR_FILE" "FINREC_BASE_URL is required"
assert_python "no-actions"
rm -rf "$LAST_CASE_DIR"
unset FINREC_BASE_URL

FINREC_OWNER_USERNAME=""
run_case "no_actions" 2
export LAST_LOG_FILE LAST_STDOUT_FILE LAST_STDERR_FILE
assert_contains "$LAST_STDERR_FILE" "FINREC_OWNER_USERNAME is required"
assert_python "no-actions"
rm -rf "$LAST_CASE_DIR"
unset FINREC_OWNER_USERNAME

FINREC_OWNER_PASSWORD=""
run_case "no_actions" 2
export LAST_LOG_FILE LAST_STDOUT_FILE LAST_STDERR_FILE
assert_contains "$LAST_STDERR_FILE" "FINREC_OWNER_PASSWORD is required"
assert_python "no-actions"
rm -rf "$LAST_CASE_DIR"
unset FINREC_OWNER_PASSWORD

FINREC_EXPECTED_TARGET=""
run_case "no_actions" 2
export LAST_LOG_FILE LAST_STDOUT_FILE LAST_STDERR_FILE
assert_contains "$LAST_STDERR_FILE" "FINREC_EXPECTED_TARGET is required"
assert_python "no-actions"
rm -rf "$LAST_CASE_DIR"
unset FINREC_EXPECTED_TARGET

FINREC_SMOKE_MODE="rollback"
FINREC_CONFIRM_ROLLBACK_SMOKE=""
run_case "no_actions" 2
export LAST_LOG_FILE LAST_STDOUT_FILE LAST_STDERR_FILE
assert_contains "$LAST_STDERR_FILE" "FINREC_CONFIRM_ROLLBACK_SMOKE=yes"
assert_python "no-actions"
rm -rf "$LAST_CASE_DIR"
unset FINREC_SMOKE_MODE FINREC_CONFIRM_ROLLBACK_SMOKE
