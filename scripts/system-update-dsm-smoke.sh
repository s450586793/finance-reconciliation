...
readonly web_repository="ghcr.io/s450586793/finance-reconciliation-web"
...
if config_image not in (old_reference, f"{repository}@{digest}"):
    raise SystemExit(1)
rollback_reference = f"finance-reconciliation-rollback-web:{task_id}"
...
check_confirmation() {
  [ "${FINREC_CONFIRM_SYSTEM_UPDATE:-}" = "yes" ] || {
    fail "error: explicit confirmation is required via FINREC_CONFIRM_SYSTEM_UPDATE=yes" 2
  }
  case "$smoke_mode" in
    success) ;;
    rollback)
      [ "${FINREC_CONFIRM_ROLLBACK_SMOKE:-}" = "yes" ] || {
        fail "error: rollback smoke requires FINREC_CONFIRM_ROLLBACK_SMOKE=yes" 2
      }
      ;;
    *)
      fail "error: FINREC_SMOKE_MODE must be success or rollback" 2
      ;;
  esac
}
...
smoke_mode="${FINREC_SMOKE_MODE:-success}"
check_confirmation
require_value FINREC_BASE_URL
require_value FINREC_OWNER_USERNAME
require_value FINREC_OWNER_PASSWORD
require_value FINREC_APP_DIR
require_value FINREC_EXPECTED_TARGET
require_version FINREC_EXPECTED_TARGET

expected_target="$FINREC_EXPECTED_TARGET"
base_url="${FINREC_BASE_URL%/}"
compose_project="${FINREC_COMPOSE_PROJECT:-finance-reconciliation}"
app_dir="$FINREC_APP_DIR"
compose_file="$app_dir/compose.yml"
env_file="$app_dir/.env"
state_file="${FINREC_UPDATER_STATE_FILE:-/state/update-state.json}"
...
create_form_body "$login_body" "$FINREC_OWNER_USERNAME" "$FINREC_OWNER_PASSWORD" "$login_csrf" "/system/update/"
