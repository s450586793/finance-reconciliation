...
cp "$project_root/scripts/public_scan.py" "$source_repo/scripts/public_scan.py"
...
GIT_AUTHOR_NAME="Inherited Hostile Author" \
GIT_AUTHOR_EMAIL="inherited-author@example.invalid" \
GIT_COMMITTER_NAME="Inherited Hostile Committer" \
GIT_COMMITTER_EMAIL="inherited-committer@example.invalid" \
FINREC_PUBLIC_SENSITIVE_ANCHORS_FILE="$anchors" \
...
expected_identity='Finance Reconciliation Snapshot <snapshot@example.invalid>
Finance Reconciliation Snapshot <snapshot@example.invalid>'
[ "$(git -C "$destination" show -s --format='%an <%ae>%n%cn <%ce>' HEAD)" = \
  "$expected_identity" ] || \
  fail_test "snapshot must override inherited author and committer identities"
...
scripts/public-snapshot-manifest.txt
scripts/public_scan.py
scripts/scan-public-tree.py'
[ "$(git -C "$destination" ls-tree -r --name-only HEAD)" = "$expected_paths" ] || \
  fail_test "snapshot must contain only manifest-selected paths"
