...
requested_confirmation=${FINREC_CONFIRM_UPDATER_UPDATE-}
requested_target_tag=${FINREC_UPDATER_IMAGE_TAG-}
rejected_ambient=0
...
exec "$os_tcb_env" -i \
  PATH=/usr/bin:/usr/local/bin:/bin \
  FINREC_CONFIRM_UPDATER_UPDATE="$requested_confirmation" \
  FINREC_UPDATER_IMAGE_TAG="$requested_target_tag" \
  FINREC_UPDATER_REJECTED_AMBIENT="$rejected_ambient" \
  FINREC_UPDATER_PYTHON_BINARY="$python_binary" \
  FINREC_UPDATER_DOCKER_BINARY="$docker_binary" \
  FINREC_UPDATER_MKTEMP_BINARY="$mktemp_binary" \
  FINREC_UPDATER_MKDIR_BINARY="$mkdir_binary" \
  FINREC_UPDATER_CHMOD_BINARY="$chmod_binary" \
  FINREC_UPDATER_CMP_BINARY="$cmp_binary" \
  FINREC_UPDATER_RM_BINARY="$rm_binary" \
  FINREC_UPDATER_SLEEP_BINARY="$sleep_binary" \
  FINREC_UPDATER_COMPOSE_PLUGIN="$compose_plugin" \
  "$python_binary" -I -S "$atomic_helper" lock-exec \
  "$bash_binary" "$0" "$@" <<'FINREC_UPDATER_BASH'
set +x
set -euo pipefail
...
readonly compose_project="finance-reconciliation"
readonly app_dir="/volume4/docker/docker/finance-reconciliation/app"
readonly env_file="$app_dir/.env"
readonly compose_file="$app_dir/compose.yml"
readonly docker_socket="unix:///var/run/docker.sock"
readonly updater_repository="ghcr.io/s450586793/finance-reconciliation-updater"
...
[ -n "${FINREC_UPDATER_PYTHON_BINARY:-}" ] || exit 1
[ -n "${FINREC_UPDATER_DOCKER_BINARY:-}" ] || exit 1
[ -n "${FINREC_UPDATER_MKTEMP_BINARY:-}" ] || exit 1
[ -n "${FINREC_UPDATER_MKDIR_BINARY:-}" ] || exit 1
[ -n "${FINREC_UPDATER_CHMOD_BINARY:-}" ] || exit 1
[ -n "${FINREC_UPDATER_CMP_BINARY:-}" ] || exit 1
[ -n "${FINREC_UPDATER_RM_BINARY:-}" ] || exit 1
[ -n "${FINREC_UPDATER_SLEEP_BINARY:-}" ] || exit 1
[ -n "${FINREC_UPDATER_COMPOSE_PLUGIN:-}" ] || exit 1
readonly python_binary="$FINREC_UPDATER_PYTHON_BINARY"
readonly docker_binary="$FINREC_UPDATER_DOCKER_BINARY"
readonly mktemp_binary="$FINREC_UPDATER_MKTEMP_BINARY"
readonly mkdir_binary="$FINREC_UPDATER_MKDIR_BINARY"
readonly chmod_binary="$FINREC_UPDATER_CHMOD_BINARY"
readonly cmp_binary="$FINREC_UPDATER_CMP_BINARY"
readonly rm_binary="$FINREC_UPDATER_RM_BINARY"
readonly sleep_binary="$FINREC_UPDATER_SLEEP_BINARY"
readonly compose_plugin="$FINREC_UPDATER_COMPOSE_PLUGIN"
...
readonly requested_confirmation="${FINREC_CONFIRM_UPDATER_UPDATE:-}"
readonly requested_target_tag="${FINREC_UPDATER_IMAGE_TAG:-}"
[ "${FINREC_UPDATER_REJECTED_AMBIENT:-1}" = "0" ] || exit 1
[ "$EUID" -eq 0 ] || exit 1
...
scratch_dir="$("$mktemp_binary" -d /var/tmp/finance-reconciliation-updater-update.XXXXXX 2>/dev/null)" || exit 1
