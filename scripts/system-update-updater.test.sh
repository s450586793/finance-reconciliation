...
original_path="$PATH"
suite_dir="$(mktemp -d)"
production_lock_file="/run/finance-reconciliation-system-update-updater.lock"
fixed_root="/volume4/docker/docker/finance-reconciliation"
...
cleanup_fixed_fixture() {
  if [ "$fixed_fixture_created" -eq 1 ]; then
    sudo -n /volume4/.finance-reconciliation-test-bin/python3 -I -S - \
      "$fixed_root_identity" \
      "$fixed_app_identity" \
      "$fixed_marker_identity" \
      "$fixed_marker_value" \
...
try:
    ...
    marker_link = os.stat(
        ".finance-reconciliation-updater-test-owner",
        dir_fd=root_descriptor,
        follow_symlinks=False,
    )
    ...
    if root_entries != {"app", ".finance-reconciliation-updater-test-owner"}:
        raise SystemExit(1)
    ...
    os.unlink(".finance-reconciliation-updater-test-owner", dir_fd=root_descriptor)
    os.fsync(root_descriptor)
    os.close(root_descriptor)
    root_descriptor = -1
    os.rmdir("finance-reconciliation", dir_fd=parent_descriptor)
...
SH
  ...
  if [ -d /volume4/.finance-reconciliation-updater-fake ] && [ -s /volume4/.finance-reconciliation-updater-fake/docker.log ]; then
...
real_python = "/volume4/.finance-reconciliation-test-bin/python3"
control_scenario = "/volume4/.finance-reconciliation-updater-fake/scenario"
scenario = (
...
try:
    ...
    if inject_atomic_fault and scenario in {
        ...
        specification = importlib.util.spec_from_file_location("finrec_updater_atomic", helper_path)
...
try:
    ...
    if scenario in validation_scenarios and is_updater_validation:
        with open("/volume4/.finance-reconciliation-updater-fake/docker.log", "a", encoding="ascii") as handle:
            handle.write(
                f'{{"action":"updater-validation","phase":"{validation_phase}","status":{validation_status}}}\n'
            )
        os.chmod("/volume4/.finance-reconciliation-updater-fake/docker.log", 0o600)
PY
...
Path("/volume4/.finance-reconciliation-updater-fake/untrusted-python-called").touch()
...
if os.environ.get("FINREC_FAKE_DOCKER_PREFLIGHT") != "1":
    private_parent = None
...
if CONTROL_PATH.is_dir():
    ...
    ENV_PATH = Path("/volume4/docker/docker/finance-reconciliation/app/.env")
    RAW = "Traceback private-token private-password sha256:" + "f" * 64 + " private-container-id /volume4/docker/docker/finance-reconciliation/app/.env daemon raw failure"
else:
...
REPOSITORY = "ghcr.io/s450586793/finance-reconciliation-updater"
...
COMPOSE_PREFIX = [
    ...
    "--env-file", "/volume4/docker/docker/finance-reconciliation/app/.env",
    "-f", "/volume4/docker/docker/finance-reconciliation/app/compose.yml",
]
...
def container_payload(container_id: str, up_count: int) -> dict[str, object]:
    if container_id == DB_ID:
        return {
            ...
            "Config": {
                "Image": "postgres:16",
                "Labels": {
                    "com.docker.compose.project": "finance-reconciliation",
    ...
    if container_id == WEB_ID:
        ...
        return {
            ...
            "Config": {
                "Image": "ghcr.io/s450586793/finance-reconciliation-web:v1.2.3",
                "Labels": {
                    "com.docker.compose.project": "finance-reconciliation",
    ...
    if is_recovery and SCENARIO == "recovery_ref_mismatch":
        image_ref = "ghcr.io/s450586793/finance-reconciliation-updater:v9.9.9"
...
if args == ["image", "inspect", NEW_REF]:
    ...
    if SCENARIO == "env_race":
        ENV_PATH.unlink()
        ENV_PATH.write_bytes(b"FINREC_UPDATER_IMAGE_TAG=v9.9.9\nATTACK=must-remain\n")
...
is_updater_scratch = (
    sys.argv[1:] == ["-d", "/var/tmp/finance-reconciliation-updater-update.XXXXXX"]
)
if scenario != "lock_concurrency" or not is_updater_scratch:
    os.execv("/volume4/.finance-reconciliation-test-bin/mktemp", ["/volume4/.finance-reconciliation-test-bin/mktemp", *sys.argv[1:]])
...
is_scratch_cleanup = (
    ...
    and Path(sys.argv[3]).name.startswith("finance-reconciliation-updater-update.")
)
...
if not is_success_cleanup:
    os.execv("/volume4/.finance-reconciliation-test-bin/rm", ["/volume4/.finance-reconciliation-test-bin/rm", *sys.argv[1:]])
...
assert_no_public_leak() {
  ...
  for forbidden in \
    ...
    /volume4/docker/docker/finance-reconciliation/app \
    /run/finance-reconciliation-system-update-updater.lock \
    /private/ \
...
prepare_fake_control() {
  local scenario="$1"
  if [ -d /volume4/.finance-reconciliation-updater-fake ]; then
    printf '%s' "$scenario" >/volume4/.finance-reconciliation-updater-fake/scenario
    : >/volume4/.finance-reconciliation-updater-fake/docker.log
    printf '{"up":0}' >/volume4/.finance-reconciliation-updater-fake/docker-state.json
    /usr/bin/rm -f -- \
      /volume4/.finance-reconciliation-updater-fake/lock-owner-ready \
      /volume4/.finance-reconciliation-updater-fake/lock-owner-release \
      /volume4/.finance-reconciliation-updater-fake/lock-second-mktemp-called
  fi
}
...
run_update_case() {
  ...
  environment=(
    ...
    "FINREC_UPDATER_TOKEN=private-token"
    ...
    "RAW_SENTINEL=Traceback private-token private-password sha256:ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff private-db-container-id /volume4/docker/docker/finance-reconciliation/app/.env daemon raw failure"
  )
  case "$invocation_mode" in
  ...
  if [ -d /volume4/.finance-reconciliation-updater-fake ]; then
    LAST_LOG_FILE="/volume4/.finance-reconciliation-updater-fake/docker.log"
    LAST_STATE_FILE="/volume4/.finance-reconciliation-updater-fake/docker-state.json"
  else
...
wait_for_private_path() {
  /volume4/.finance-reconciliation-test-bin/python3 -I -S - "$1" <<'PY'
import sys
import time
from pathlib import Path
...
run_lock_concurrency_case() {
  ...
  first_pid=$!
  set -e
  if ! wait_for_private_path /volume4/.finance-reconciliation-updater-fake/lock-owner-ready; then
    : >/volume4/.finance-reconciliation-updater-fake/lock-owner-release
  ...
  second_status=$?
  set -e
  [ ! -e /volume4/.finance-reconciliation-updater-fake/lock-second-mktemp-called ] || second_reached_mktemp=1
  [ ! -s /volume4/.finance-reconciliation-updater-fake/docker.log ] || docker_before_release=1
  scratch_before_release="$(find /var/tmp -mindepth 1 -maxdepth 1 -name 'finance-reconciliation-updater-update.*' -print -quit)"
  : >/volume4/.finance-reconciliation-updater-fake/lock-owner-release
  set +e
  wait "$first_pid"
  first_status=$?
...
assert_pretransaction_lock_rejection() {
  ...
  [ -z "$(find /var/tmp -mindepth 1 -maxdepth 1 -name 'finance-reconciliation-updater-update.*' -print -quit)" ] || {
...
run_terminated_lock_owner() {
  ...
  updater_pid="$(/usr/bin/sudo -n /usr/bin/cat /volume4/.finance-reconciliation-updater-fake/lock-owner-ready)"
  ...
  [ ! -s /volume4/.finance-reconciliation-updater-fake/docker.log ] || fail_test "terminated lock owner reached Docker"
  [ -z "$(find /var/tmp -mindepth 1 -maxdepth 1 -name 'finance-reconciliation-updater-update.*' -print -quit)" ] || {
...
PY
  ...
  environment=(
    ...
    "FINREC_HOME_PLUGIN_SENTINEL=$plugin_sentinel"
  ...
  if [ "$status" -ne 0 ]; then
    ...
    if [ -s /volume4/.finance-reconciliation-updater-fake/docker.log ]; then
      docker_boundary="last Docker action: $(/usr/bin/tail -n 1 /volume4/.finance-reconciliation-updater-fake/docker.log)"
    fi
...
: >"${FINREC_BASH_ENV_SENTINEL:?}"
SH
  cat >"$case_dir/malicious-bin/bash" <<'SH'
#!/bin/sh
: >"${FINREC_PATH_BASH_SENTINEL:?}"
exit 97
SH
  cat >"$case_dir/malicious-bin/docker" <<'SH'
#!/bin/sh
: >"${FINREC_BOOTSTRAP_DOCKER_SENTINEL:?}"
exit 97
SH
  ...
  sudo -n /usr/bin/env -i \
    ...
    FINREC_BASH_ENV_SENTINEL="$bash_env_sentinel" \
    FINREC_PATH_BASH_SENTINEL="$path_bash_sentinel" \
    FINREC_BOOTSTRAP_DOCKER_SENTINEL="$docker_sentinel" \
    "$script_path" \
    >"$case_dir/stdout" \
    2>"$case_dir/stderr"
  status=$?
...
if [ "${FINREC_TEST_BATCH:-}" = "BOOTSTRAP" ]; then
  run_clean_bootstrap_case
  printf 'system-update-updater clean bootstrap contract tests passed\n'
  exit 0
fi

if [ "${FINREC_TEST_NAMESPACE:-}" != "1" ]; then
...
if [ "${FINREC_TEST_BATCH:-}" = "ROOTFS" ]; then
...
read -r \
  ...
  fixed_marker_identity \
  fixed_marker_value <<<"$(
  sudo -n /volume4/.finance-reconciliation-test-bin/python3 -I -S - "$fixed_root" <<'PY'
import os
import secrets
import sys
from pathlib import Path
...
marker_path = root_path / ".finance-reconciliation-updater-test-owner"
...
if [ "${FINREC_TEST_BATCH:-}" = "LOCK" ]; then
  ...
  /usr/bin/sudo -n /volume4/.finance-reconciliation-test-bin/rm -f -- "$production_lock_file"
...
if [ "${FINREC_TEST_BATCH:-}" = "RETENTION" ]; then
  printf '# Token=private-token\r\nFINREC_UPDATER_IMAGE_TAG=v1.2.3\r\nOPAQUE=private-password' >"$suite_dir/original.env"
  printf '# Token=private-token\r\nFINREC_UPDATER_IMAGE_TAG=v1.2.4\r\nOPAQUE=private-password' >"$suite_dir/target.env"
  printf 'services: {}\n' >"$suite_dir/compose.yml"
  sudo -n install -o root -g root -m 0644 "$suite_dir/compose.yml" "$fixed_app_dir/compose.yml"
  mapfile -t evidence_before < <(
    sudo -n find /var/tmp -mindepth 1 -maxdepth 1 -type d -name 'finance-reconciliation-updater-update.*' -print
  )
...
if [ "${FINREC_TEST_BATCH:-}" = "CLEANUP_RM" ]; then
  printf '# Token=private-token\r\nFINREC_UPDATER_IMAGE_TAG=v1.2.3\r\nOPAQUE=private-password' >"$suite_dir/original.env"
  printf '# Token=private-token\r\nFINREC_UPDATER_IMAGE_TAG=v1.2.4\r\nOPAQUE=private-password' >"$suite_dir/target.env"
  printf 'services: {}\n' >"$suite_dir/compose.yml"
  sudo -n install -o root -g root -m 0644 "$suite_dir/compose.yml" "$fixed_app_dir/compose.yml"
...
if [ "${FINREC_TEST_BATCH:-}" = "TCB" ]; then
  printf '# Token=private-token\r\nFINREC_UPDATER_IMAGE_TAG=v1.2.3\r\nOPAQUE=private-password' >"$suite_dir/original.env"
  printf '# Token=private-token\r\nFINREC_UPDATER_IMAGE_TAG=v1.2.4\r\nOPAQUE=private-password' >"$suite_dir/target.env"
  printf 'services: {}\n' >"$suite_dir/compose.yml"
  sudo -n install -o root -g root -m 0644 "$suite_dir/compose.yml" "$fixed_app_dir/compose.yml"
  run_update_case "tcb_preflight" any
  [ ! -e /volume4/.finance-reconciliation-updater-fake/untrusted-python-called ] || {
...
if [ "${FINREC_TEST_BATCH:-}" = "DSM_TCB" ]; then
...
if [ "${FINREC_TEST_BATCH:-}" = "B_CLEANUP" ]; then
...
run_rejected_case \
  ...
  root \
  FINREC_CONFIRM_UPDATER_UPDATE=yes \
  FINREC_UPDATER_IMAGE_TAG=v01.2.3
...
printf 'FINREC_UPDATER_IMAGE_TAG=v1.2.2\nFINREC_UPDATER_IMAGE_TAG=v1.2.3\n' >"$suite_dir/duplicate.env"
...
for invalid_tag in v01.2.3 v1.02.3 v1.2.03 v1.2 v1.2.3.4 1.2.3 latest 'v1.2.3 '; do
  ...
  run_rejected_case \
    ...
    root \
    FINREC_CONFIRM_UPDATER_UPDATE=yes \
    "FINREC_UPDATER_IMAGE_TAG=$invalid_tag"
done
...
assert_fixed_update_failure
printf 'FINREC_UPDATER_IMAGE_TAG=v9.9.9\nATTACK=must-remain\n' >"$suite_dir/raced.env"
