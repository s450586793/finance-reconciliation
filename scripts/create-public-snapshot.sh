...
[ "$#" -eq 1 ] || fail_closed
[ -n "${FINREC_PUBLIC_SENSITIVE_ANCHORS_FILE:-}" ] || fail_closed
...
anchor_path="$(realpath -e -- "$FINREC_PUBLIC_SENSITIVE_ANCHORS_FILE" 2>/dev/null)" || fail_closed
...
GIT_AUTHOR_NAME="Finance Reconciliation Snapshot" \
GIT_AUTHOR_EMAIL="snapshot@example.invalid" \
GIT_COMMITTER_NAME="Finance Reconciliation Snapshot" \
GIT_COMMITTER_EMAIL="snapshot@example.invalid" \
git -C "$destination" \
  -c core.hooksPath=/dev/null \
  -c commit.gpgsign=false \
  -c user.name="Finance Reconciliation Snapshot" \
