...
LOCK_PATH = Path("/run/finance-reconciliation-system-update-updater.lock")
...
def capture_environment(arguments: list[str]) -> None:
    ...
    for index, complete_line in enumerate(lines):
        ...
        seen.add(key)
        if key == "FINREC_UPDATER_IMAGE_TAG":
            updater_index = index
            old_tag = value
    ...
    lines[updater_index] = f"FINREC_UPDATER_IMAGE_TAG={target_tag}{ending}"
...
def replace_environment(arguments: list[str]) -> None:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            dir=path.parent,
            prefix=".finance-reconciliation-system-update-updater.",
            delete=False,
        ) as temporary_file:
