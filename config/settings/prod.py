...
def _production_updater_url() -> str:
    value = _required_value("FINREC_UPDATER_URL")
    if value != "http://updater:8090":
        raise ImproperlyConfigured(
            "FINREC_UPDATER_URL must be http://updater:8090 in production"
        )
    return value


def _production_updater_token() -> str:
    value = os.environ.get("FINREC_UPDATER_TOKEN", "")
    if not value.strip() or len(value.encode("utf-8")) < 32:
        raise ImproperlyConfigured(
            "FINREC_UPDATER_TOKEN must be a non-empty token of at least 32 bytes in production"
        )
    return value
...
FINREC_RELEASE_VERSION = _required_value("FINREC_RELEASE_VERSION")
if (
    re.fullmatch(
        r"v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)",
        FINREC_RELEASE_VERSION,
    )
    is None
):
    raise ImproperlyConfigured(
        "FINREC_RELEASE_VERSION must use canonical vX.Y.Z format in production"
    )
FINREC_UPDATER_URL = _production_updater_url()
FINREC_UPDATER_TOKEN = _production_updater_token()
DATABASES = {
