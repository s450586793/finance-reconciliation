...
def navigation(request):
    ...
    is_finance = user is not None and user_has_role(user, Role.FINANCE)
    can_read_ledger = is_finance or (
        user is not None and user_has_role(user, Role.OWNER)
    )
    items = [
        {
            "label": "总览",
    ...
    if user is not None and user_has_role(user, Role.OWNER):
        items.append(
            {
                "label": "系统设置",
